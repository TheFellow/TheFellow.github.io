...
The stored format is a navigation projection, not a compiler-event archive. It retains repository documents; package, type, function, method, test, and provider-defined anchors; and relationships such as imports, dependencies, implementations, tests, generation, documentation, exposure, links, and embeds. It omits occurrences, statement-level calls and references, containment duplication, fields, constants, locals, and broad content-token postings. Providers may emit richer facts during indexing, but the projection decides what is worth paying to retain.

That boundary changed the physical and agent-facing shape substantially. On the recorded 3,019,968-byte Go repository, format 4 reduced the database from 1,034,616,832 bytes to 16,777,216 bytes, a 98.38% reduction and 5.56 times the non-Git source size. The representative `explore --json` response fell from 171,537 bytes to 3,157 bytes, and its selected `context --json` response was 3,195 bytes. These are one reproducible audit, not universal performance claims, but they establish the practical acceptance boundary the implementation now tests.

The native Go provider and optional .NET, Python, Rust, C/C++, TypeScript/JavaScript, JVM, and Universal Ctags adapters still use their language-native tools to establish identities and navigation facts. The open process protocol keeps those runtimes outside the Go core. Explicit permissions, capability claims, input routing, validation, and atomic publication prevent an adapter from silently broadening its authority. The compact projection is shared across providers, so adding a precise compiler does not imply retaining all of its event exhaust forever.

The workspace provider gives the same treatment to repositories that do not compile. It indexes Git-visible paths and structured Markdown, front matter, headings, links, assets, routes, and metadata without executing the site. Documents and sections are found through `explore` or `symbols`, opened through `context`, and connected through retained graph kinds such as `documents`, `links-to`, and `embeds`. The older posting- and containment-dependent `workspace` command family is intentionally gone. A source-only provider adds bounded facts from Protobuf, OpenAPI, GraphQL, PostgreSQL migrations, Terraform, and declarative project manifests without running builds, generators, package managers, databases, or network loaders.
