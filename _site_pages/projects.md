...
<div class="feature-tiles">
  {% assign apex = site.site_pages | where: "permalink", "/apex/" | first %}
  <a class="feature-tile feature-tile--featured" href="{{ apex.permalink | relative_url }}" style="--feature-accent: {{ apex.accent }};">
    <header class="feature-tile__header">
      ...
      <span>
        <span class="feature-tile__eyebrow">{{ apex.language }}</span>
        <span class="feature-tile__title">{{ apex.title }}</span>
      </span>
    </header>
    <span class="feature-tile__description">{{ apex.excerpt }}</span>
    <span class="feature-tile__topics">
      {% for topic in apex.topics %}<span>{{ topic }}</span>{% endfor %}
    </span>
    <span class="feature-tile__updated">Recreated {{ apex.last_updated | date: "%B %-d, %Y" }}</span>
    <span class="feature-tile__cta">Launch Apex <span class="feature-tile__arrow" aria-hidden="true">→</span></span>
  </a>
