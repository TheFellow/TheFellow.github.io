<!-- Generated from https://thefellow.github.io/articles/building-high-quality-software/ by scripts/generate_llm_content.py; do not edit. -->

# Building High-Quality Software

Source: [https://thefellow.github.io/articles/building-high-quality-software/](https://thefellow.github.io/articles/building-high-quality-software/)

## Pyramid summary

- **~2 words:** Architecture lessons
- **~8 words:** Eleven lessons that turn architectural intent into executable software constraints.
- **Expanded:** A preview of eleven lessons about turning architectural intent into executable constraints, using Mixology as the worked example.

## Full content

**Part 1 of [Building Mixology](/series/mixology.md).**
...
In Mixology, the request enters the same application pipeline whether it came from the CLI, TUI, or GUI. The ingredient module loads the current entity, Cedar authorizes the transition and any proposed replacement, and the command records the historically named `IngredientDeleted` event. Generated dispatch sends that retirement fact to Drinks, Inventory, Menus, and Orders. Drinks rewrites future recipes or marks them for review, Inventory removes unusable stock, Menus recomputes retained item availability, and Orders block historical snapshots that still name the retired ingredient. Every handler works through its own persistence boundary, all of them share the original transaction, and none can emit another event. Integration tests exercise the whole path.

That single operation contains most of the series. The lessons below pull it apart, one decision at a time.

<figure class="article-figure">
  <img src="/assets/images/articles/mixology/lesson-map.png" alt="An ordered map of eleven Mixology lessons, moving from boundaries and types through the application pipeline, events, generation, errors, authorization, surfaces, owned seams, system tests, and deliberate limits on complexity.">
...
`IngredientDeleted` carries the retired ingredient, time, and optional permanent replacement with its conversion ratio. The generated dispatcher gives that fact independently to Drinks, Inventory, Menus, and Orders. Drinks and Menus discover affected recipes and menu items before Inventory removes stock or Drinks rewrites recipes. Before any handler mutates data, every handler with a preparation phase runs its [`Handling`](https://github.com/TheFellow/go-modular-monolith/blob/main/app/domains/menus/handlers/ingredient-deleted.go) method. Only then does dispatch begin applying changes. This prevents the first mutation from erasing information a later handler still needs to query.

All of that work remains inside the transaction opened for the retirement. If any recipe rewrite, stock removal, menu availability update, or order block fails, the ingredient, dependent state, and audit record return to their previous state together. The event separates knowledge between domains without giving up immediate consistency.
...
The focused guide [Turning Cross-Domain Calls into Enforced Boundaries](/articles/turning-cross-domain-calls-into-enforced-boundaries.md) follows this deletion from the tempting direct-call implementation through owned reactions, transactional dispatch, package rules, and integration evidence.
...
For a detailed treatment of the TUI architecture introduced in this section, continue with [Building an Application TUI Toolkit](/articles/building-an-application-tui-toolkit.md).
...
composed desktop lifecycle. The [third-surface audit](/articles/using-a-third-surface-as-an-architecture-test.md)
found application leaks precisely where the CLI and TUI had shared an assumption, while the
[bespoke-view boundary](/articles/bespoke-views-over-a-shared-application-boundary.md) kept each runtime's
...
The ingredient retirement tests create ingredients, inventory, drinks, orders, and published menus, then retire one ingredient through the public module. Without a replacement, a required recipe remains inspectable and moves to `review_required`, the pending order becomes blocked, stock disappears, and the published menu retains its item while reporting degraded availability. With a compatible permanent replacement, the current recipe is rewritten while the historical order snapshot continues to name the retired ingredient. Those tests are compact because the architecture is already doing the orchestration. They do not recreate that orchestration with mocks.
