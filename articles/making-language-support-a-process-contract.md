...
The host request includes the required `navigation-v1` profile. `run.begin` repeats the negotiated provider and fact encoding. Each `unit.begin` declares one atomically replaceable semantic unit. `facts` frames carry documents, declaration symbols, and navigation edges in bounded batches. Occurrences, lexical search terms, and call, reference, definition, or containment edges are outside this profile. `unit.end` supplies exact received counts, and `run.end` supplies the complete duplicate-free unit inventory.

The adapter is responsible for compiler-backed truth within that deliberately narrow contract. The host filters the streamed batches and projects complete units again before publication, so an older or third-party producer cannot expand the managed index by sending discarded fact kinds.
...
The .NET adapter uses Roslyn and MSBuild to select exact C# declaration anchors and compiler-evaluated project relationships. FSharp.Compiler.Service supplies typed F# anchors. Both now emit the common navigation profile directly rather than serializing statement-level references and calls that the host would discard. Omitting unsupported evidence is still more accurate than filling the provider stream with a guess.

Rust and C++ take a second route through the same contract. `weave-rust` supervises `rust-analyzer scip`, while `weave-cpp` supervises `scip-clang` and projects their output into declaration anchors and retained navigation relationships. C++ facts are exact for one selected compilation database and Clang version; the adapter refuses zero or multiple discovered databases rather than blending incompatible build variants.

Repository-scale Rust validation made the root and ownership rules concrete. The adapter discovers the nearest nested Cargo manifest or `rust-project.json`, runs rust-analyzer at that build root, and rebases its SCIP documents to repository-relative paths. It retains one deterministic declaration anchor for equivalent global records and only relationships allowed by `navigation-v1`; conflicting descriptions still reject the run.
...
| --- | --- |
| Compiler-accepted module, class, function, method, and type-alias anchors | `exact` |
| Import and dependency statements | `declared` |
| Parameters, locals, repeated bindings, references, and calls | omitted |
...
Every completed run may replace only facts owned by its provider. The host rejects a document, symbol, or edge whose provider differs from the enclosing unit and run. A broad syntactic adapter therefore cannot relabel compiler facts, and a compiler upgrade cannot delete document relationships owned by the workspace provider.
