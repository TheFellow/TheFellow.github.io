<!-- Generated from https://thefellow.github.io/articles/growing-mixology-with-fyne/ by scripts/generate_llm_content.py; do not edit. -->

# Growing Mixology with a GUI Surface

Source: [https://thefellow.github.io/articles/growing-mixology-with-fyne/](https://thefellow.github.io/articles/growing-mixology-with-fyne/)

## Pyramid summary

- **~2 words:** GUI expansion
...
**Part 6 of [Building Mixology](/series/mixology.md).**
...
**Current workflow scope (September 13, 2026):** the [cross-domain surface alignment](https://github.com/TheFellow/go-modular-monolith/blob/0d5e64b0455f7a5c96d4afada64654a5fdbb9a2c/docs/surface-parity.md) adds native GUI/TUI interactions for substitution administration, single and batch order amendments, initial stock receipt, quarantine, release, disposal, and movement history. Details distinguish accepted history from approved fulfillment and expose one command activity covering its domain and leaf effects. The original experiment checkpoints remain below; the September follow-up records the expanded coverage.
...
The first implementation slice chose [Fyne 2.8.0](https://github.com/TheFellow/go-modular-monolith/blob/5af00d09571e03a21665774258f8f3d32b111d16/go.mod) and added a dedicated desktop executable. The [application-facing rename](https://github.com/TheFellow/go-modular-monolith/pull/34) exposed that entry point at `main/gui`, while the reusable framework mechanics initially lived in `pkg/fyne`. The current package is [`pkg/toolkits/gui`](https://github.com/TheFellow/go-modular-monolith/tree/0d5e64b0455f7a5c96d4afada64654a5fdbb9a2c/pkg/toolkits/gui): GUI names its architectural purpose, while Fyne remains its implementation technology. Its composition root opens the shared `data/mixology.db` database used by the CLI and TUI, places the diagnostic log in the platform's per-user configuration directory, constructs the application and persistent session, and installs an idempotent close path on the native window. The test uses Fyne's in-memory application and virtual window, navigates the composed shell, closes it twice, and then reopens the same database to prove that ownership was released.
...
[Projecting Actions Across User Interfaces](/notes/projecting-actions-across-user-interfaces.md) describes the shared projection contract. The [TUI toolkit guide](/articles/building-an-application-tui-toolkit.md) and this journal cover only how each runtime consumes it.
...
Tags exposed a mutation boundary. A form can save domain state and a complete tag set, but two sequential commands can leave the first committed when the second fails. The historical [`RunTaggedMutation` implementation](https://github.com/TheFellow/go-modular-monolith/commit/c565fd6e95c5db781eaefb110c04944cf3188830) initially solved that by validating tags and composing two mutations in one store transaction. That helper has since been removed. Current TUI and GUI forms pass an optional `tag.Edit` to one consuming-domain command, preserving independent parsing, validation, rendering, and form state.

The original helper was protected at two levels. Behavioral tests forced tag replacement failure and asserted that the domain change and audit entry rolled back. Historical AST-based [TUI wiring](https://github.com/TheFellow/go-modular-monolith/commit/5f71a3151fe733a1ca1fc8045450d7e2a4f0d4bf) and [GUI wiring](https://github.com/TheFellow/go-modular-monolith/commit/5f71a3151fe733a1ca1fc8045450d7e2a4f0d4bf) contracts enumerated all fifteen tagged mutation paths to prevent bypassing the helper. Current wiring and rollback tests protect the domain-owned command contract described in the September follow-up.
...
- Desktop composition consumes each domain's projected collection capability and omits denied routes from navigation and dashboard cards. Entity-filtered catalogs expose public collection entry because Cedar authorizes and elides every returned row; Audit list and Tag summary project their genuine authorization resources. This hides Audit and Tags from a sommelier, for example, without inventing synthetic entities or bypassing row policy. Non-permission failures leave the route visible so the workspace can report the actual problem.
...
[The historical atomic helper received a review of its own](https://github.com/TheFellow/go-modular-monolith/commit/c565fd6e95c5db781eaefb110c04944cf3188830). `RunTaggedMutation` then joined a caller-owned transaction, and a rollback test checked that both domain state and tags disappeared on rollback. The wiring contracts also inspected mutation callbacks. This was an intermediate design, now superseded by one domain-owned command per transaction: nested commands and sequential commands sharing a caller-owned transaction are rejected. Tagged confirmation tests continue to check that tag choice advances into the domain confirmation.
...
| Audit | Show each command's changed and referenced entities and leaf field changes; label failed effects as uncommitted attempts |

Forms retain the revision shown when editing begins, and failed submissions preserve the draft or amendment queue. Initial receipt uses revision zero, so two editors cannot overwrite each other's first stock entry. Batch submission calls `Orders.AmendBatch`, one command that validates and authorizes the selection and emits the complete `OrdersAmended` delta for Inventory, Orders, and Menus. Retirement calls `Ingredients.Retire` separately. Each owns its transaction and audit activity, so a failed retirement leaves a previously approved batch committed.

The [domain-owned command revision](https://github.com/TheFellow/go-modular-monolith/blob/0d5e64b0455f7a5c96d4afada64654a5fdbb9a2c/docs/transactional-workflows.md) also replaces the tagged helper with calls such as `Ingredients.Update(ctx, ingredient, tag.Replace(desired, expected))`. A nil desired set preserves tags; an empty non-nil set clears them. Ingredients emits its own `TagsReplaced` event with its authorization actions. Tagging validates, checks the expected set, and authorizes both before and after tags in `Handling`, then persists associations in `Handle`. A tag veto rolls back every domain and leaf effect. There is one audit activity for the command; workflow correlation is no longer written or displayed.
...
Behavioral tests cover permissions, captured revisions, batch rollback, keyboard input, and semantic widget controls. The [render review commands](https://github.com/TheFellow/go-modular-monolith/blob/0d5e64b0455f7a5c96d4afada64654a5fdbb9a2c/docs/surface-parity.md#review-captures) exercise the composed clients and write GUI PNGs and terminal ANSI frames for inspection:
...
- [x] Fourteen arch-lint rules enforce surface isolation, matching toolkit access, toolkit independence, cross-domain ownership, explicit internal consumers, and command boundaries; isolated negative fixtures prove representative allowed and denied imports.
...
- [Using a Third Surface as an Architecture Test](/articles/using-a-third-surface-as-an-architecture-test.md) extracts the parity audit into a repeatable architecture technique.
- [Bespoke Views over a Shared Application Boundary](/articles/bespoke-views-over-a-shared-application-boundary.md) explains why the shared asset is application behavior rather than a universal view model.
- [Testing Native Go Desktop Applications Headlessly](/articles/testing-native-go-desktop-applications-headlessly.md) turns the testing ladder into a practical desktop strategy.
- [Authorization Is Part of Navigation](/articles/authorization-is-part-of-navigation.md) follows Cedar decisions through routes, aggregates, rows, and actions.
