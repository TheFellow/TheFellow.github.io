...
Domain facades enter typed generic methods such as `pipeline.Command`, `pipeline.LoadCommand`, and `pipeline.Query`. Commands authorize trusted input and resulting state, then complete event reactions and success auditing before commit. The [generic-methods note](/notes/go-1-27-generic-methods-and-the-mixology-pipeline/) records the API's development; the [deck](/talks/building-mixology/) teaches the current execution path.
...
The [.NET semantic port](/projects/modular-monolith/) provides a parallel implementation of the seven-context architecture using .NET 10, EF Core, Terminal.Gui, and .NET MAUI. Its parity ledger describes the Go baseline that it ports. The newer Go amendment and stock-lifecycle work is not an automatic claim of parity with that separate repository.
