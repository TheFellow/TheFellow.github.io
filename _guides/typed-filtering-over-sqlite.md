...
redirect_from:
  - /articles/typed-filtering-over-bstore/
  - /guides/typed-filtering-over-bstore/
  - /guides/typed-filtering-over-sqlite/
series: mixology
series_order: 11
order: 37
...
icon: "filter"
accent: "#f59f00"
topics: ["Typed filters", "Expr", "SQLite"]
---
...
The first version of this work targeted bstore. Mixology has since [moved its embedded persistence boundary to SQLite](/articles/migrating-mixology-from-bstore-to-sqlite/). That migration changed the query adapter and its pushdowns, but it did not change the expression language callers use or the rule that the complete expression remains the semantic authority.
...
The .NET Mixology port consumes the published [Expr for .NET](/projects/expr-dotnet/) 0.1.0 package and applies the same division with EF Core. Its planner emits an `Expression<Func<TRow, bool>>` for safe constraints, hydrates fields such as tags, then evaluates the complete Expr program against the public view. The mechanisms differ, but the correctness rule is the same.
