...
The current `navigation-v1` adapter profile aligns authority with retention. An adapter emits declaration anchors and supported navigation relationships, while the host filters and validates the complete streamed inventory before publication. Installing a more precise compiler does not silently opt the worktree into producing or storing every occurrence, call, reference, field, or local it can report.
