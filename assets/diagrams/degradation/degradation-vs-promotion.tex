...
\begin{tikzpicture}[text=ink,state/.style={rounded corners=4pt,draw=blue,fill=panel,very thick,minimum width=3.25cm,minimum height=1.1cm,align=center},arrow/.style={-{Latex[length=3mm]},very thick},note/.style={font=\small,align=center,text=ink}]
 ...
 \node[state,draw=amber,right=3cm of published] (degraded) {Published\\degraded};
 \draw[arrow,amber] (published)--node[above=.12cm,note,text width=2.55cm]{ingredient retired\\without\\\mbox{replacement}}(degraded);
 \node[note,right=.55cm of degraded,text width=2.7cm] {membership and history\\remain visible};
 \node[state,below=2.25cm of published] (draft) {Draft\\serviceable};
 \node[state,draw=red,right=3cm of draft] (blocked) {Draft\\known blocker};
 ...
 \node[font=\bfseries\large,text=ink] at (3.1,1.65) {Degradation preserves truth; promotion requires readiness};
\end{tikzpicture}
\end{document}
